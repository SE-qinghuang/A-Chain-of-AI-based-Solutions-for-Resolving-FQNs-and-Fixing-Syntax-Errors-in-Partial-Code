...
result_path = "Content_representation_change_example_again.csv"
...
# 只对 "ERROR_For_Code_Fix" 列非空的行进行处理
df_for_Code_Fix = pd.read_csv("Content_representation_change_example.csv", encoding='utf-8')
not_null_rows = df_for_Code_Fix.loc[df_for_Code_Fix["ERROR_For_Code_Fix"].notnull()]

for index, row in tqdm.tqdm(not_null_rows.iterrows()):
    ...
    # 附加在csv文件后面
    df_for_Code_Fix.loc[index, 'simplenames'] = simplenames
    ...
    # 附加在csv文件后面
    df_for_Code_Fix.loc[index, 'FQNs'] = FQNs
    with_import = FQN_Supplement(row["without_import"], FQNs)
    # 附加在csv文件后面
    df_for_Code_Fix.loc[index, 'with_import'] = with_import
    Error_Message = compile_code(with_import, row["filename"])
    # 附加在csv文件后面
    if Error_Message == "True" or Error_Message == "can't decode error message":
        if Error_Message == "can't decode error message":
            df_for_Code_Fix.loc[index, 'ERROR_For_With_import'] = "can't decode error message"
        else:
            df_for_Code_Fix.loc[index, 'ERROR_For_With_import'] = "True"
        continue
    else:
        df_for_Code_Fix.loc[index, 'ERROR_For_With_import'] = Error_Message
    ...
    # 附加在csv文件后面
    df_for_Code_Fix.loc[index, 'ERROR_MESSAGE_EXPLANATION'] = ERROR_MESSAGE_EXPLANATION
    ...
    # 附加在csv文件后面
    df_for_Code_Fix.loc[index, 'Code_Fix'] = Code_Fix
    # 再次编译
    ...
    if Error_Message == "True" or Error_Message == "can't decode error message":
        ...
        df_for_Code_Fix.loc[index, 'ERROR_For_Code_Fix'] = Error_Message
    # 保存到csv文件
    df_for_Code_Fix.to_csv(result_path, index=False, encoding='utf-8')
# 统计Error_For_Code_Fix列中True的个数, 并计算准确率
accuracy = df_for_Code_Fix["ERROR_For_With_import"].value_counts(normalize=True).loc["True"]
